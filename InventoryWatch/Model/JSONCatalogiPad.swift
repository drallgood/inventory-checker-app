//
//  JSONCatalogiPad.swift
//  InventoryWatch
//

import Foundation

enum JSONCatalogiPad {
    ...
    struct iPadRootLite: Codable { let discovered_models: [String: [String: Node]]?; let country_mappings: [String: CountryMap]? }

    @MainActor private static func iPadModelJSONFiles() -> [URL] {
        ...
        let filtered = candidates.filter { url in
            ...
            return lc.hasPrefix("ipad-") && lc.hasSuffix("-intl")
        }
        return filtered
    }
    ...
    @MainActor static func categoriesSourcePages(for country: Country) -> [String] {
        ...
        for url in iPadModelJSONFiles() {
            guard let data = try? Data(contentsOf: url) else { continue }
            if let root = try? JSONDecoder().decode(iPadRootLite.self, from: data) {
    ...
    @MainActor static func tokenDisplayName(for country: Country, sourcePage: String) -> String? {
        ...
        let uc = country.shortcode.uppercased()
        for url in iPadModelJSONFiles() {
            guard let data = try? Data(contentsOf: url), let root = try? JSONDecoder().decode(iPadRootLite.self, from: data) else { continue }
